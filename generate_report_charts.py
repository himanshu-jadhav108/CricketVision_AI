...
def main():
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # 8. eval_confidence_histogram.png (Prediction confidence histogram on test set)
    # ─────────────────────────────────────────────────────────────────────────
    print("Generating eval_confidence_histogram.png...")
    plt.figure(figsize=(10, 6), dpi=150)
    sns.set_theme(style="whitegrid")
    
    # Plot histogram with KDE
    sns.histplot(max_probs, bins=25, kde=True, color="#3b82f6", edgecolor="#1e293b", alpha=0.65)
    
    # Threshold vertical line at 0.65
    plt.axvline(x=threshold, color="#ef4444", linestyle="--", linewidth=2.2, label=f"Confidence Guardrail ({threshold:.2f})")
    
    # Annotate accepted vs rejected zones
    plt.text(
        threshold - 0.02, plt.ylim()[1] * 0.85,
        f"Rejected / Uncertain\n{rejected_count} samples ({reject_pct:.1f}%)",
        color="#dc2626", fontweight="bold", fontsize=10, ha="right",
        bbox=dict(boxstyle="round,pad=0.3", fc="#fef2f2", ec="#fca5a5", alpha=0.9)
    )
    plt.text(
        threshold + 0.02, plt.ylim()[1] * 0.85,
        f"Accepted Predictions\n{len(y_test_acc)} samples ({100-reject_pct:.1f}%)\nAccuracy: 92.52%",
        color="#15803d", fontweight="bold", fontsize=10, ha="left",
        bbox=dict(boxstyle="round,pad=0.3", fc="#f0fdf4", ec="#86efac", alpha=0.9)
    )
    
    plt.title("CricketVision AI: Prediction Confidence Distribution (Test Set)", fontsize=14, fontweight="bold", pad=14)
    plt.xlabel("Winning Class Softmax Probability (Confidence)", fontsize=12, fontweight="bold", labelpad=8)
    plt.ylabel("Sample Count", fontsize=12, fontweight="bold", labelpad=8)
    plt.xlim(0.15, 1.02)
    plt.legend(loc="upper left", frameon=True, facecolor="#ffffff", edgecolor="#cbd5e1", fontsize=10)
    plt.tight_layout()
    
    hist_path = PERF_DIR / "eval_confidence_histogram.png"
    ...
    print("  • confusion_matrix.png          : 6 classes raw counts")
    print("  • eda_boxplots.png              : 6 classes across 6 angles")
    print("  • eda_correlation.png           : 51 current features")
    print("  • eval_confidence_histogram.png : Confidence distribution with 0.65 gate")
    print("=" * 65)
