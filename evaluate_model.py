...
CSV_REPORT_PATH = os.path.join(PROJECT_ROOT, 'reports', 'confusion_matrix.csv')
...
"""
    ...
    # Save post-threshold confusion matrix CSV
    cm = confusion_matrix(y_test_acc, base_preds_acc)
    cm_df = pd.DataFrame(cm, index=encoder.classes_, columns=encoder.classes_)
    cm_df.to_csv(CSV_REPORT_PATH)
        
    print(f"Report written successfully to {REPORT_PATH}")
    print(f"Confusion matrix CSV written to {CSV_REPORT_PATH}")
